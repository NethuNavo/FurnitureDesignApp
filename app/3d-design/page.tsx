...
type StoredFurniture = {
  ...
  type: string;
  name?: string;
  // In the 3D viewer we treat these as meters (2D pixels are converted on load).
  x: number;
  y: number;
  width: number; // meters
...
export default function ThreeDDesignPage() {
  // wrap with protected route at render time
  ...
  // read query parameter on client side
  useEffect(() => {
  ...
  // sourceDesignId is now managed with state above
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (parsed?.room) {
        const convertedFurniture = parsed.furniture.map(item => ({
          ...item,
          x: item.x / pxPerMeter,
          y: item.y / pxPerMeter,
        }));
        const converted = { ...parsed, furniture: convertedFurniture };
        timer = window.setTimeout(() => {
          setStoredDesign(converted);
        }, 0);
      }
    } catch {
    ...
    (async () => {
      try {
        ...
        if (!res.error && res.data?.design) {
          const payload = res.data.design.payload as StoredDesign;
          const convertedFurniture = payload.furniture.map(item => ({
            ...item,
            x: item.x / pxPerMeter,
            y: item.y / pxPerMeter,
          }));
          setStoredDesign({ ...payload, furniture: convertedFurniture });
        }
      } catch {
        // ignore
      }
  ...
  useEffect(() => {
    ...
    for (const item of furniture) {
      ...
      // Furniture positions are already normalized to meters when we load the design.
      // Clamp top-left meters to the valid room area.
  ...
  return (
    <ProtectedRoute>
      <PageLayout showBackground={false} className="bg-[#d8cec4] text-[#372414]">
      {/* Full-page 3D viewer shell. */}
              ...
              {isFromSavedDesign ? (
                <>
                  <Button asChild variant="neutral" size="sm">
                    <Link href={backToSavedHref}>Back to Saved Designs</Link>
                  </Button>
                  <Button asChild variant="secondary" size="sm">
                    <Link href={editIn2DHref}>Edit in 2D</Link>
                  </Button>
                </>
              ) : (
                  <Button asChild variant="secondary" size="sm">
                    <Link href={editIn2DHref}>Back to 2D</Link>
                  </Button>
              )}
            </div>
